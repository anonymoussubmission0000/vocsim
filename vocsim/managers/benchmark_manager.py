    ...
    def run_subset_benchmarks(
        ...
        for feature_name, feature_hdf5_path in feature_iter:
            ...
            for bench_conf in self.benchmark_configs:
                ...
                needs_features = isinstance(benchmark, ClassificationBenchmark) or (isinstance(benchmark, ClusteringPurity) and not getattr(benchmark, "use_dist_matrix", False))
                needs_distances = isinstance(benchmark, (PerceptualAlignment, PrecisionAtK, FValueBenchmark, CSCFBenchmark, ClassSeparationRatio)) or (isinstance(benchmark, ClusteringPurity) and getattr(benchmark, "use_dist_matrix", True))
