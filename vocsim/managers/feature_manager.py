...
class FeatureManager:
    ...
    def get_feature_file_path(self, feature_name: str, dataset_cache_id: str, subset_features_dir: Optional[Path] = None) -> Optional[Path]:
        ...
        if needs_intermediate and not needs_pca:
            final_prefix = "intermediate_features"
    ...
    def _extract_and_save_features_hdf5(
        ...
        """
        Extracts features using single-scan/batched-write. Saves to HDF5.
        ...
        Returns:
            Optional[Dict[str, Any]]: Metadata dictionary containing 'original_indices' list, or None on failure.
        """
        ...
        logger.info("Step 1: Scanning dataset for valid items and shape...")
        ...
        total_items_scan = None

        if not is_iterable:
            try:
                total_items_scan = len(dataset)
            except TypeError:
                pass
            ...
            for item in batch:
                ...
                if is_valid_item:
                    ...
                    if max_shape is None:
                        try:
                            features = extractor.extract(audio_data=audio_array, sample_rate=sample_rate, **params)
                            if features is not None:
                                ...
                                if features_np.size > 0 and not np.isnan(features_np).any() and not np.isinf(features_np).any():
                                    max_shape = features_np.shape
                                    final_ndim = features_np.ndim
                                    final_dtype = np.float32
                                    logger.info("Determined target shape: %s, dtype: %s (from item %d)", max_shape, final_dtype, current_internal_idx)
                                    del features, features_np
                                else:
                                    logger.warning("First item feature extraction yielded empty/NaN/Inf. Shape undetermined.")
                                    is_valid_item = False
                        except Exception as shape_err:
                            logger.warning("Error extracting first item feature for shape: %s. Shape undetermined.", shape_err)
                            is_valid_item = False

                if is_valid_item:
                    original_idx = current_internal_idx
                    valid_item_original_indices.append(original_idx)
                ...
                global_idx_scan += 1
            pbar_scan.set_postfix({"Valid": len(valid_item_internal_indices), "Skipped": skipped_scan_count})
        pbar_scan.close()

        processed_count = len(valid_item_internal_indices)
        logger.info("Scan complete. Found %d valid items to process.", processed_count)
        if processed_count == 0:
            logger.error("No valid audio items found for '%s'. Aborting.", feature_name)
            return None
        if max_shape is None:
            logger.error("Failed to determine target feature shape for '%s'. Aborting.", feature_name)
            return None

        logger.info("Step 3: Creating HDF5 file '%s' for %d items...", cache_path.name, processed_count)
        ...
        try:
            ...
            h5_dataset = h5_file.create_dataset(
                HDF5_DATASET_NAME,
                shape=(processed_count, *max_shape),
                dtype=final_dtype,
                compression="gzip",
                compression_opts=4,
                shuffle=True,
            ...
            logger.info("Created HDF5 feature dataset with shape %s", h5_dataset.shape)

            for k, v in metadata_for_attrs.items():
                try:
                    h5_dataset.attrs[k] = json.dumps(v, cls=NpEncoder)
                except TypeError:
                    logger.warning("Could not JSON serialize attribute '%s'. Skipping.", k)
                    h5_dataset.attrs[k] = "SERIALIZATION_ERROR"

            if valid_item_original_indices:
                indices_array = np.array(valid_item_original_indices, dtype=np.int64)
                h5_file.create_dataset(HDF5_INDICES_NAME, data=indices_array, compression="gzip", compression_opts=4)
                logger.info("Saved '%s' dataset with shape %s", HDF5_INDICES_NAME, indices_array.shape)

        except Exception as e:
            logger.error("Failed to create HDF5 file %s: %s", cache_path, e, exc_info=True)
            if h5_file:
                h5_file.close()
            return None

        logger.info("Step 4: Writing %d features to HDF5...", processed_count)
        write_idx = 0
        ...
        total_batches_write = len(loader_write) if hasattr(loader_write, "__len__") else None
        pbar_write = tqdm(loader_write, desc=f"Write [{feature_name}]", total=total_batches_write, leave=False)
        global_idx_write = 0

        try:
            for batch in pbar_write:
                features_to_write_list: List[np.ndarray] = []
                batch_start_write_idx = write_idx
                ...
                for item in batch:
                    ...
                    try:
                        ...
                        if isinstance(features, torch.Tensor):
                            ...
                            raise TypeError(f"Invalid feature type: {type(features)}")

                        if features_np.size == 0:
                            raise ValueError("Extractor returned empty features")
                        if np.isnan(features_np).any() or np.isinf(features_np).any():
                            raise ValueError("Features contain NaN/Inf")
                        if features_np.dtype != final_dtype:
                            features_np = features_np.astype(final_dtype)

                        pad_width = [(0, max_shape[i] - features_np.shape[i]) for i in range(final_ndim)]
                        if any(p[1] < 0 for p in pad_width):
                            raise ValueError(f"Feature shape {features_np.shape} exceeds target {max_shape}")
                        ...
                        logger.error("Processing item internal_idx %d failed: %s", current_internal_idx, e)
                        error_occurred_write = True

                if features_to_write_list:
                    try:
                        batch_array = np.stack(features_to_write_list)
                        num_in_batch = batch_array.shape[0]
                        batch_end_write_idx = batch_start_write_idx + num_in_batch

                        if batch_end_write_idx > processed_count:
                            raise IndexError(f"Write end index {batch_end_write_idx} exceeds total {processed_count}")
                        if batch_array.shape != (num_in_batch, *max_shape):
                            raise ValueError(f"Stacked batch shape {batch_array.shape} mismatch expected {(num_in_batch, *max_shape)}")

                        h5_dataset[batch_start_write_idx:batch_end_write_idx] = batch_array
                        write_idx = batch_end_write_idx

                    except (ValueError, IndexError) as stack_write_err:
                        logger.error("Failed to stack or write batch (start_idx=%d, num=%d): %s. Skipping batch write.", batch_start_write_idx, len(features_to_write_list), stack_write_err)
                        error_occurred_write = True
                    except Exception as h5_write_err:
                        logger.error("Failed to write batch to HDF5: %s", h5_write_err, exc_info=True)
                        error_occurred_write = True
            ...
            if write_idx != processed_count:
                logger.error("Final write count %d != expected valid count %d. HDF5 file may be incomplete or contain errors.", write_idx, processed_count)
                error_occurred_write = True

        except Exception as e:
            logger.error("Critical error during Write Pass for '%s': %s", feature_name, e, exc_info=True)
            error_occurred_write = True
        finally:
            if h5_file:
                h5_file.close()
                logger.debug("Closed HDF5 file: %s", cache_path)
            if "features_to_write_list" in locals():
                del features_to_write_list
            gc.collect()
            if self.device.type == "cuda":
                try:
                    torch.cuda.empty_cache()
            ...
            if error_occurred_write:
                logger.error("Errors occurred during write pass. Deleting potentially incomplete HDF5 file: %s", cache_path)
                try:
                    if cache_path.exists():
                        cache_path.unlink()
                except OSError as unlink_err:
                    logger.error("Failed to delete incomplete HDF5 file %s: %s", cache_path, unlink_err)
                return None
    ...
    def _process_intermediate_features(
        ...
        if base_metadata is None:
            ...
            return None
        base_original_indices = load_hdf5(base_feature_path, HDF5_INDICES_NAME, "Base Indices", allow_missing=True)
    ...
    def _process_pca_features(
        ...
        intermediate_original_indices = load_hdf5(intermediate_feature_path, HDF5_INDICES_NAME, "Intermediate Indices", allow_missing=True)
        ...
        try:
            pca_model_path = get_cache_path(pca_model_base_dir, "pca_model", dataset_cache_id, feature_config, file_extension=".joblib")
    ...
    def _check_cache(
        ...
        Args:
            prefix (str): Prefix for the cache file (e.g., 'features', 'intermediate_features').
        ...
        indices = load_hdf5(expected_path, HDF5_INDICES_NAME, f"{feature_name} indices", allow_missing=True)
    ...
    def process_subset_features(
        ...
        for feature_config in configs_to_process:
            ...
            if not base_feature_name or not needs_intermediate or needs_pca:
                continue

            effective_config = self._get_effective_feature_config(feature_config, current_subset_key)
            path_to_use, cached_metadata = self._check_cache("intermediate_features", subset_features_dir, dataset_cache_id, effective_config, total_items_in_subset, check_attr="num_items")
            ...
            if path_to_use and cached_metadata:
                ...
                expected_inter_save_path = get_cache_path(subset_features_dir, "intermediate_features", dataset_cache_id, effective_config)
        ...
        for feature_config in configs_to_process:
            ...
            effective_config = self._get_effective_feature_config(feature_config, current_subset_key)
            path_to_use, cached_metadata = self._check_cache("features", subset_features_dir, dataset_cache_id, effective_config, total_items_in_subset, check_attr="num_items")
            ...
            if path_to_use and cached_metadata:
                ...
                pca_input_path: Optional[Path] = None
                intermediate_feature_name = None
                if needs_intermediate:
                    if feature_name in available_intermediate_paths:
                        pca_input_path = available_intermediate_paths[feature_name]
                    else:
                        logger.warning("Required intermediate feature for '%s' not found in available paths. Trying to compute it if needed.", feature_name)
                        base_feature_path_pca = available_base_paths.get(base_feature_name)
                        if not base_feature_path_pca:
                            logger.error("Base feature '%s' unavailable for on-demand intermediate creation needed by PCA '%s'.", base_feature_name, feature_name)
                            continue
                        inter_conf = effective_config.copy()
                        inter_conf.pop("pca", None)
                        inter_conf.pop("pca_load_chunks", None)
                        expected_inter_path_pca = get_cache_path(subset_features_dir, "intermediate_features", dataset_cache_id, inter_conf)
                        inter_meta_pca = self._process_intermediate_features(base_feature_path_pca, expected_inter_path_pca, inter_conf)
                        if inter_meta_pca:
                            pca_input_path = expected_inter_path_pca
                            available_intermediate_paths[feature_name] = expected_inter_path_pca
                        else:
                            logger.error("On-demand intermediate creation failed for PCA '%s'.", feature_name)
                            continue

                else:
                    pca_input_path = available_base_paths.get(base_feature_name)

                if not pca_input_path:
                    logger.error("Cannot compute PCA '%s': Input feature (Base/Intermediate) '%s' not available.", feature_name, base_feature_name if not needs_intermediate else feature_name + " (intermediate)")
                    continue
