...
def apply_averaging(features_np: np.ndarray, method: Optional[str]) -> Optional[np.ndarray]:
    ...
    Args:
        features_np (np.ndarray): Input feature array. Can be multi-dimensional,
                                  but averaging methods generally expect 2D [Batch, Dim].
        method (Optional[str]): The averaging method to apply. Supported methods:
                                'mean_row_col', 'first_row_col', 'first_row', 'first_col'.
                                If None or unsupported, the input is simply flattened.

    Returns:
        Optional[np.ndarray]: The averaged feature array (always 2D [1, N]) or None if an unrecoverable error occurs.
    """
    original_shape = features_np.shape
    ndim = features_np.ndim
    func_name = "apply_averaging"
    allowed_methods = {"mean_row_col", "first_row_col", "first_row", "first_col"}

    if method is None or method not in allowed_methods:
        if method is not None:
            logger.warning("[%s] Invalid or unsupported method '%s'. Allowed: %s. Flattening input.", func_name, method, allowed_methods)
        else:
            logger.debug("[%s] No averaging method specified. Flattening input shape %s.", func_name, original_shape)
        if ndim == 0:
            return features_np.reshape(1, 1).astype(np.float32)
        return features_np.reshape(1, -1).astype(np.float32)
    ...
    if ndim != 2:
        logger.warning(
            "[%s] Method '%s' requires 2D input, got %dD shape %s. Reshaping to 2D by combining all dimensions except the last.",
            func_name,
            method,
            ndim,
            original_shape,
        ...
        if any(s == 0 for s in original_shape):
            logger.warning("[%s] Input has zero-sized dimension in shape %s. Returning empty [1, 0].", func_name, original_shape)
    ...
    if n_rows == 0 or n_cols == 0:
        logger.warning("[%s] Cannot apply method '%s' to empty 2D array %s. Returning empty [1, 0].", func_name, method, features_np.shape)
        return np.empty((1, 0), dtype=np.float32)

    result_1d = None
    try:
        if method == "mean_row_col":
            ...
            if n_rows == 0:
                raise IndexError("Cannot take first row from empty array.")
            result_1d = features_np[0, :].astype(np.float32)
        elif method == "first_col":
            if n_cols == 0:
                raise IndexError("Cannot take first col from empty array.")
            result_1d = features_np[:, 0].astype(np.float32)

    except IndexError as e:
        logger.error("[%s] Index Error applying '%s' to shape %s: %s. Flattening input.", func_name, method, features_np.shape, e)
        return features_np.reshape(1, -1).astype(np.float32)
    except Exception as e:
        logger.error("[%s] Unexpected error applying '%s' to shape %s: %s. Flattening input.", func_name, method, features_np.shape, e, exc_info=True)
        return features_np.reshape(1, -1).astype(np.float32)

    if result_1d is None:
        logger.error("[%s] Result is None after applying method '%s' (unexpected internal error). Flattening input.", func_name, method)
        return features_np.reshape(1, -1).astype(np.float32)

    final_result = result_1d.reshape(1, -1).astype(np.float32)
